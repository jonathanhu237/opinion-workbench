    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            if version < 37:
                ...
                version = 37
            if version < 38:
                from opinion_workbench_api.services.media_cleanup import (
                    purge_managed_media,
                )
...
def _migrate_to_version_33(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.text_insufficient_report_v33 import migrate
...
def _migrate_to_version_34(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.text_only_prompt_v34 import migrate
...
def _migrate_to_version_35(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.search_content_metadata_v35 import migrate
...
def _migrate_to_version_30(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.search_total_limit import migrate
...
def _migrate_to_version_31(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.multi_platform_v31 import migrate
...
def _migrate_to_version_32(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.text_only_cache_v32 import migrate
...
def _migrate_to_version_14(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.topic_reports import migrate
...
def _migrate_to_version_37(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.prompt_guards_v37 import migrate
...
def _migrate_to_version_38(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.media_exit_v38 import migrate
...
def _migrate_to_version_39(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.report_pacing_v39 import migrate
...
def _migrate_to_version_36(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.search_run_ordering_v36 import migrate
...
def _migrate_to_version_15(connection: sqlite3.Connection) -> None:
    ...
    """
    from opinion_workbench_api.migrations.automation_workflows import migrate
...
def _migrate_to_version_16(connection: sqlite3.Connection) -> None:
    ...
    from opinion_workbench_api.migrations.topic_reports_v16 import migrate
...
def _migrate_to_version_17(connection: sqlite3.Connection) -> None:
    ...
    from opinion_workbench_api.migrations.automation_workflows_v17 import migrate
...
def _migrate_to_version_18(connection: sqlite3.Connection) -> None:
    ...
    from opinion_workbench_api.migrations.automation_workflows_v18 import migrate
...
def _migrate_to_version_19(connection: sqlite3.Connection) -> None:
    ...
    from opinion_workbench_api.migrations.search_runs_v19 import migrate
...
def _migrate_to_version_24(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.media_cache_v24 import migrate
...
def _migrate_to_version_26(connection: sqlite3.Connection) -> None:
    """Remove abandoned platform data and enforce the Weibo-only product."""
    from opinion_workbench_api.migrations.weibo_only_v26 import migrate
    ...
    try:
        # v26 replaces the six historical platform-bearing tables so their
        # final SQLite CHECK constraints are genuinely Weibo-only.  SQLite
        # cannot drop/recreate parent tables with FK enforcement enabled;
        # migration runs the complete referential check before restoring the
        # connection's normal setting below.
        connection.execute("PRAGMA foreign_keys = OFF")
        connection.execute("PRAGMA legacy_alter_table = ON")
        connection.execute("BEGIN IMMEDIATE")
        version = _read_user_version(connection)
...
def _migrate_to_version_27(connection: sqlite3.Connection) -> None:
    ...
    from opinion_workbench_api.migrations.search_term_diagnostics_v27 import migrate
...
def _migrate_to_version_28(connection: sqlite3.Connection) -> None:
    ...
    from opinion_workbench_api.migrations.search_run_status_v28 import migrate
...
def _migrate_to_version_21(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.report_generations_v21 import migrate
...
def _migrate_to_version_20(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.manual_reports_v20 import migrate
...
def _migrate_to_version_13(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.collection_schedules import migrate
...
def _migrate_to_version_12(connection: sqlite3.Connection) -> None:
    from opinion_workbench_api.migrations.initial_analysis import migrate
...
def _migrate_to_version_10(connection: sqlite3.Connection) -> None:
    ...
    from opinion_workbench_api.search_checkpoints import backfill_legacy_completions
...
def _migrate_to_version_1(connection: sqlite3.Connection) -> None:
    ...
    try:
        ...
        # A new installation deliberately starts with no monitoring rules.
        # Operators must choose their own collection objects; historical
        # business seeds are never recreated by a fresh schema.
...
def _migrate_to_version_7(connection: sqlite3.Connection) -> None:
    """Add durable batch orchestration without rewriting runs."""
    connection.execute("BEGIN IMMEDIATE")
    try:
        version = _read_user_version(connection)
        if version >= 7:
