...
from opinion_workbench_api.repositories.collection_schedules import OccurrenceClaim
...
from opinion_workbench_api.schemas.collection_schedules import OccurrenceReason
...
from opinion_workbench_api.services.platform_access import PlatformCooldownActiveError
from opinion_workbench_api.services.search_runs import (
    MAX_SEARCH_TERMS,
    SearchRunError,
    SearchRunService,
    _to_result,
        ...
        *,
        ...
        async with self._lock:
            ...
            try:
                ...
                if self._platform_access is not None:
                    scheduled_kwargs["platform_access_snapshot"] = await database_call(
                        self._platform_access.snapshot
                    )
                try:
    ...
    async def _resume_control(
        ...
        try:
            ...
            async with self._lock:
                ...
                permit = getattr(self._platform_access, "permit_explicit_resume", None)
        ...
        try:
            ...
            if (
                ...
                if callable(ensure_snapshot):
                    await database_call(ensure_snapshot, batch_id, materialized)
            ...
            while True:
                if self._shutdown_started or self._cancelling:
                    return
                item = await database_call(self._repository.next_queued_item, batch_id)
                if item is None:
