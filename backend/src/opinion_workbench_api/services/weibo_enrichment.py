...
from opinion_workbench_api.services.media_inventory import media_inventory
...
from opinion_workbench_api.services.weibo_dom import document, text_of
from opinion_workbench_api.services.weibo_media import allowed_media_url, transfer_media
        ...
        try:
                    ...
                    for _ in range(MAX_REDIRECT_HOPS + 1):
                        ...
                        async with client.stream(
                            ...
                            if overflow and response.status_code < 400:
                                body = b""
                                response_headers["x-opinion-workbench-error"] = (
                                    "detail_limit"
                                    if request.stage == "detail"
                                    else "media_limit"
                                )
            ...
            if outcome == "platform_blocked_or_rate_limited" and (
                ...
                await self._block_platform(
                    ...
                    manual_challenge_required=_payload_has_challenge(detail_payload),
                )
            return EnrichmentWorkerResult(
                outcome,
