...
from opinion_workbench_api.schemas.monitoring_rules import (
    MonitoringRule,
...
def compose_monitoring_terms(
    ...
    if not monitoring_objects:
        raise _invalid_rule("请至少输入一个采集对象。")
    if len(monitoring_objects) > MAX_TERMS_PER_RULE:
        raise _invalid_rule("每条监控规则最多包含 100 个采集对象。")
    if len(issue_keywords) > MAX_TERMS_PER_RULE:
        raise _invalid_rule("每条监控规则最多包含 100 个舆情词。")
    if len(monitoring_objects) * max(1, len(issue_keywords)) > MAX_TERMS_PER_RULE:
        raise _invalid_rule(
            "每条监控规则最多生成 100 个搜索词，请减少采集对象或舆情词。"
        )

    objects = _validated_terms(monitoring_objects, "采集对象")
    issues = _validated_terms(issue_keywords, "舆情词")
    terms = (
