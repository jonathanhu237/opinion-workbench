...
describe('AI settings form', () => {
  ...
  it('shows loading then three labeled fields with an in-field saved-key mask', async () => {
    renderSettings()
    ...
    expect(password).toHaveValue(savedKeyMask)
    ...
    expect(screen.queryByText(/已配置，留空即可保留/)).toBeNull()
  ...
  it('keeps the mask on focus and allows typing a replacement directly', async () => {
    const user = userEvent.setup()
    renderSettings()
    const password = await screen.findByLabelText('API Key')
    expect(password).toHaveValue(savedKeyMask)
  ...
  it('requires a newly entered key for an endpoint change and prevents testing dirty values', async () => {
    ...
    expect(screen.getByLabelText('API Key')).toHaveValue(savedKeyMask)
    expect(screen.getByRole('button', { name: '测试连接' })).toBeDisabled()
  ...
  it('saves directly, restores the mask on reopen, and retains only non-secret query data', async () => {
    ...
    expect(reopenedPassword).toHaveValue(savedKeyMask)
  ...
  it('offers read retry after a storage failure', async () => {
    ...
    expect(await screen.findByLabelText('API Key')).toHaveValue(savedKeyMask)
  })
