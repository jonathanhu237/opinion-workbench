...
产品只支持微博，采集使用项目自有的原生浏览器边界；仅启动应用或打开页面不会启动浏览器。
...
当前产品采集运行时只开放微博。后端会自动使用项目自有的原生微博采集器；应用中不会展示其他平台入口。
...
请在这个窗口中登录微博；扫码、验证码和安全验证也由你在该窗口中完成。

- 无需在日常 Chrome 中开启远程调试或批准本应用连接。应用不会接管日常浏览器，
  也不会从中复制 Cookie。
- 专用窗口使用独立的本地目录 `runtime/browser/managed-chrome`，由 Chrome 保留登录态。
  该目录已排除在 Git 之外，不应共享或提交；微博登录过期后仍需重新登录。
- 微博登录检查、采集和分析中的浏览器操作共用专用窗口。单个任务结束不会关闭整个窗口；
