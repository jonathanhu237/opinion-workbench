...
Scope terminology update: the revised parent removes standalone screening. The per-item decision/evidence contract below is executed only inside an explicitly requested summary operation. A later synthetic text test and the 2026-08-28 isolated three-video probe provide limited live evidence; product media/summary integration remains unverified. See `product-integration-readiness.md`. This document's original provider research itself used no live credentials or calls.
...
- The initial reviewed Omni path requests numeric usage with documented `stream_options: {"include_usage": true}`, as exercised in the probe. Capture only bounded nonnegative integer input/output/total and recognized modality counts from completed usage events; reject booleans/coercion, handle duplicate/conflicting events deterministically and mark absent/malformed accounting unknown rather than zero. Validate totals without guessing missing modalities. Never retain arbitrary provider metadata. Preserve valid usage on subsequent local JSON/schema failure and do not weaken existing stream-success checks. Keep the synthetic text-test response/API compatible.
...
- The exercised Omni inline video form is `video_url.url = "data:;base64,<bytes>"`, retaining the MP4 audio in that same input. Do not substitute a guessed `data:video/mp4` form or make a separate audio request. This is one reviewed provider contract, not proof of support by arbitrary configured endpoints.
...
- Before strict JSON/schema validation, allow whitespace trimming and removal of only an exact enclosing whole-response JSON code fence. Reject prose-plus-JSON, multiple blocks, wrong field types/enums, trailing material and unknown keys; never extract or repair a guessed object. Product decisions remain `relevant | irrelevant | uncertain`, not the probe-only `related`. Persist stage/code and validated usage on failure, not the probe's private raw-answer excerpts. Scan validated model text for credential leakage before storing or rendering it; do not log the rejected text.
...
- Add regression cases for exact fence normalization, separate JSON/schema failure codes, usage on invalid final output, absent/duplicate/conflicting/malformed usage, literal/escaped secret sentinels and no automatic repair/retry. A cache hit contributes no new request usage; summary totals report only this operation's attempts and distinguish incomplete accounting.
