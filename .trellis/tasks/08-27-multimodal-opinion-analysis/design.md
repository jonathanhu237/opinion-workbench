...
Status: refreshed on 2026-08-28 after the accepted real-video cost experiment; fresh downstream final review pending. Checked AI configuration and delivered rule composition are unchanged.

Execution update (2026-08-28): the user subsequently approved the plan and the
clarified saved-analysis/minimal-chain approach. The summary child is active
against the checked internal media contract. Whole-five-platform media/AI live
acceptance remains open and is not a prerequisite to implementing this integration.
The single-run manual scope below is unchanged.

The delivered rule-editor contract is in `../archive/2026-08/08-27-monitoring-rule-combinations/design.md`. The probe-to-product evidence boundary and current repository baseline are in `research/product-integration-readiness.md`; neither prior rule approval nor probe acceptance establishes delivered media/summary support.
...
- Preserve the delivered editor's required `监控对象` and optional `舆情关键词` plus preview; no arbitrary Boolean editor, shared dictionary or AI keyword generation. Empty issues leave objects unchanged; otherwise compose ordinary platform queries such as `龙田街道 积水`, not guaranteed strict AND matching. The completed rule child preserves old phrases as objects with empty issues, persists both lists and returns derived effective `terms` to collectors. Keep 100-query saving/20-query execution limits, frozen snapshots and dedup. Search results remain leads; no independent screening delivery.
...
- `ai_summary_runs`: unique request UUID, source run FK, frozen context/configuration/prompt versions, state/phase, counts and timestamps, final eligible item references/input hash, optional validated composition token usage and validated structured summary. One row owns the complete manually requested pipeline and output version.
- `ai_summary_items`: ordered unique `(summary_run_id, content_id)`, frozen source projection, observation hash, enriched text/media metadata, input hash, state, model decision/reason/evidence, bounded error stage/code, optional validated attempt token usage and `reused_from_item_id` FK. No standalone screening-run tables or public screening API. Keep usage from a completed response even if its JSON/schema fails; absent usage is unknown. Reuse has no new attempt/usage and must not add the original item's historical tokens to this operation's new usage.
...
- Migrations are additive after the actual schema: configuration was v8, delivered rule composition v9, and the current working tree declares v10 for manual collection recovery. Summary adds run/item tables after the then-current version; media retrieval needs no product migration. Re-read versions before implementation, accommodate existing dirty recovery changes and never rewrite released migrations. Follow SQLite connection-per-operation, parameter binding and short transactions.
...
- A later manually resumed collection can add/update observations, but cannot mutate an existing AI run's frozen sources or evidence. Reject AI admission against an actively running/paused-for-recovery source; preserve existing browser exclusion and recovery ownership. Do not assume a failed collection will never be resumed.
...
| `POST /search-runs/{run_id}/ai-summaries` | `{request_id, force_refresh, configuration_revision}`; 202 with full frozen scope; owns analysis and composition; rejects a configuration changed since confirmation |
...
- Display provider-reported token totals compactly with unknown/partial accounting explicit; no separate cost panel or currency estimate. Count only new attempts, including local output-validation failures, and the final composition request. Reuse itself consumes no new per-item tokens, but a new composition can still consume quota. Keep stream/JSON/schema errors distinct without displaying raw responses. Only exact whole-response JSON fence normalization is permitted before strict result/citation validation.
...
- Live gates separately prove the five narrow media extractors and configured-model image/video/audio understanding. The three-Douyin-video probe informs fixtures and output/usage handling, but does not replace these gates or authorize additional calls. No key from chat, runtime database, authentication files or raw payloads enter remote sync/test artifacts.
...
- Deploy sequentially: preserve checked configuration/completed rule composition, implement internal enrichment after approval of this final plan, then activate integrated manual summary only after its dependency checks and with scope unchanged. Standalone screening is deferred. Disable/remove new UI/runners to back out while preserving collection data. Older binaries reject newer schemas, so binary downgrade requires a verified pre-migration backup; never claim automatic downgrade or delete user data.
