...
- **S6 — Traceable output.** Preserve real source references; never trust LLM-invented links/IDs, events, coverage claims or incident counts. Keep raw sources, reusable evidence and prior summary provenance intact. Report paragraphs expose compact, recognizable source hyperlinks with platform labels; source access must not depend on expanding the analysis records.
